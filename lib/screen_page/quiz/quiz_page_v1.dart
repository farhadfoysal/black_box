...
class _QuizPageV1State extends State<QuizPageV1> {
  ...
  void prevQuestion(int n) {
    setState(() {
      currentQuestionIndex = (currentQuestionIndex - n + questions.length) % questions.length;
      if (currentQuestionIndex < questions.length) {
        _shuffledAnswers = questions[currentQuestionIndex].getShuffledAnswers();
      } else {
        setState(() {
  ...
  int getCorrectCount() {
    return selectedAnswersMap.values
        .where((answer) => answer == questions[currentQuestionIndex].questionAnswers[0])
        .length;
  }
            ...
            Expanded(
              child: ListView(
                children: _shuffledAnswers.map((answer) {
                  final isSelected = selectedAnswersMap[currentQuestionIndex] == answer;
                  return QuizOption(
                    optionText: answer,
