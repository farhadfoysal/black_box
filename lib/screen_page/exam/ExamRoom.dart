...
class ExamRoom extends StatefulWidget {
  final void Function(
      String answer, String questionText, int currentQuestionIndex, int totalQuestions)
  onSelectedAnswer;
  ...
  const ExamRoom({
    ...
    required this.user,
    required this.quiz, required this.questions,
  });
...
class _ExamRoomState extends State<ExamRoom> {
    ...
    setState(() {
      if (currentQuestionIndex < widget.questions.length) {
        if (!selectedAnswersMap.containsKey(currentQuestionIndex)) {
          ...
          widget.onSelectedAnswer(answer, questionText, index,widget.questions.length);
  ...
  void nextQuestion(int n) {
    setState(() {
      currentQuestionIndex = (currentQuestionIndex + n) % widget.questions.length;
      if (currentQuestionIndex < widget.questions.length) {
        _shuffledAnswers = widget.questions[currentQuestionIndex].getShuffledAnswers();
      } else {
        setState(() {
  ...
  void prevQuestion(int n) {
    setState(() {
      currentQuestionIndex =
          (currentQuestionIndex - n + widget.questions.length) % widget.questions.length;
      if (currentQuestionIndex < widget.questions.length) {
        _shuffledAnswers = widget.questions[currentQuestionIndex].getShuffledAnswers();
      } else {
        setState(() {
  ...
  int getCorrectCount() {
    return selectedAnswersMap.values
        .where((answer) =>
    answer == widget.questions[currentQuestionIndex].questionAnswers[0])
        .length;
  }
  ...
  int getUncheckedCount() {
    return widget.questions.length - selectedAnswersMap.length;
  }
  ...
  bool isCorrectAnswer(String answer) {
    ...
    return widget.questions[inde - 1].questionAnswers[0] == answer;
  }
  ...
  Widget build(BuildContext context) {
    var currentQuestion = currentQuestionIndex < widget.questions.length
        ? widget.questions[currentQuestionIndex]
        : widget.questions[widget.questions.length - 1];
    return Scaffold(
      backgroundColor: Colors.white,
      appBar: AppBar(
        backgroundColor: Colors.white,
                    ...
                    child: GestureDetector(
                      onTap: () {
                        setState(() {
                          ...
                          _shuffledAnswers = widget.questions[currentQuestionIndex]
                              .getShuffledAnswers();
                        });
                      },
                      child: CircleAvatar(
